...
pub enum SolverMessage {
    /// Challenge to solver for evaluation
    SlotChallenge {
        epoch: u64,
        timeslot: u64,
...
impl Display for SolverMessage {
...
pub async fn run(
    timer_to_solver_rx: Receiver<SolverMessage>,
    solver_to_main_tx: Sender<ProtocolMessage>,
    plot: &Plot,
) {
    info!("Solve loop is running...");
    while let Ok(SolverMessage::SlotChallenge {
        epoch,
        timeslot,
        epoch_randomness,
        slot_challenge,
    }) = timer_to_solver_rx.recv().await
    {
